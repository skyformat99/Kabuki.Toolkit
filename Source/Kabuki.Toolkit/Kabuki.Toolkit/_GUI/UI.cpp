...
/**
 Note: moving this routine here from add_controls.cpp prevents the linker
 from touching add_controls.o in non-deprecated programs, which 
 descreases the linked size of small GLUI programs substantially (100K+). (OSL 2006/06)
*/
void Node::Add_child_To_control (Node *parent,Control *child)
{
  ...
  /** Now set the 'hidden' auto based on the parent **/
...
int Node::Add_control (Control *child)
{
...
int Main::Add_control (Node *parent, Control *control)
{
...
/************************************ CB::operator () () ************/
void CB::operator () (Control*ctrl) const
{
...
int GLUI::Init (const char *text, long flags, int x, int y, int parent_window) 
{
  ...
  /*  glut_keyboard_CB = Master.glut_keyboard_CB;
      glut_reshape_CB  = Master.glut_reshape_CB;
      glut_special_CB  = Master.glut_special_CB;
      glut_mouse_CB    = Master.glut_mouse_CB;*/


  if ((flags & SUBWINDOW) != SUBWINDOW) {  /* not a subwindow, creating a new top-level window */
  ...
  else /* *is* a subwindow */
  {
    ...
    /*
      glutReshapeFunc (parent_window_reshape_func);
      glutSpecialFunc (parent_window_special_func);
      glutKeyboardFunc (parent_window_keyboard_func);
      glutMouseFunc (parent_window_mouse_func);
      */
...
/**************************** Main::create_standalone_window () ********/

void Main::create_standalone_window (const char *name, int x, int y)
{
...
/******************************** Main::create_subwindow () **********/

void Main::create_subwindow (int parent_window, int window_alignment)
{
...
/**************************** Main::setup_default_glut_callbacks () *****/

void Main::setup_default_glut_callbacks (void)
{
...
void display_func (void)
{
  ...
  glui = Master.find_by_window_id (glutGetWindow ());
...
void reshape_func (int w,int h)
{
  ...
  /*printf ("reshape_func (): %d  w/h: %d/%d\n", glutGetWindow (), w, h);          */

  current_window = glutGetWindow ();

  /***  First check if this is main glut window ***/
  glut_window = Master.find_glut_window (current_window);
  if (glut_window) {
    if (glut_window->glut_reshape_CB) glut_window->glut_reshape_CB (w,h);

    /***  Now send reshape events to all subwindows  ***/
    glui = (GLUI*) Master.gluis.first_child ();
    while (glui) {
      if (TEST_AND (glui->flags, SUBWINDOW) AND 
    ...
    glui = Master.find_by_window_id (current_window);
...
void keyboard_func (unsigned char key, int x, int y)
{
  ...
  if (glut_window) { /**  Was event in a GLUT window?  **/
    if (Master.active_control_glui AND Master.active_control) {
      glutSetWindow (Master.active_control_glui->get_glut_window_id ());
      
      Master.active_control_glui->keyboard (key,x,y);    
	  finish_drawing ();
...
void special_func (int key, int x, int y)
{
  ...
  if (glut_window) /**  Was event in a GLUT window?  **/
  {
    if (Master.active_control_glui AND Master.active_control)
    {
      ...
      Master.active_control_glui->special (key,x,y);    
      finish_drawing ();
...
void mouse_func (int button, int state, int x, int y)
{
  ...
  if (glut_window) { /**  Was event in a GLUT window?  **/
    if (Master.active_control_glui != NULL) 
      Master.active_control_glui->deactivate_current_control ();
...
void idle_func (void)
{
  ...
  if (Master.glut_idle_CB) {
    ...
    /*   current_window = glutGetWindow ();          */
    /*   if (Master.gluis.first_child () != NULL) {          */
    /*      new_window = ((Main*)Master.gluis.first_child ())-> */
    /*   main_gfx_window_id;          */
...
/*********************************** Master_Object::create_glui () ******/

GLUI *Master_Object::create_glui (const char *name, long flags,int x,int y)
{
...
/************************** Master_Object::create_subwindow () ******/

GLUI *Master_Object::create_subwindow (int parent_window, 
						   long flags)
{
  GLUI *new_glui = new GLUI;
  String new_name;
  format_str (new_name, "subwin_%p", this);

  new_glui->Init (new_name.c_str (), flags | SUBWINDOW, 0,0,
		    parent_window);
  new_glui->mainPanel->set_int_val (PANEL_EMBOSSED);
  new_glui->link_this_To_parent_last (&this->gluis);
  return new_glui;
}


/********************** Master_Object::find_by_window_id () ********/

GLUI  *Master_Object::find_by_window_id (int window_id)
{
  Node *node;
...
void    Main::display (void)
{
  ...
  */
  ...
  /*  if (TEST_AND (this->flags, SUBWINDOW))
  ...
  if (win_w != mainPanel->w OR win_h != mainPanel->h) {
    glutReshapeWindow (mainPanel->w, mainPanel->h);
    return;
  }
  ...
  //  mainPanel->draw_bkgd_box (0, 0, win_w, win_h);
  mainPanel->draw_recursive (0, 0);
...
void    Main::reshape (int reshape_w, int reshape_h)
{
  int new_w, new_h;
  ...
  if (TEST_AND (this->flags, SUBWINDOW)) {
    ...
    /***** if (TEST_AND (this->flags,SUBWINDOW_LEFT)) {
      }
      else if (TEST_AND (this->flags,SUBWINDOW_LEFT)) {
      }
      else if (TEST_AND (this->flags,SUBWINDOW_LEFT)) {
      }
      else if (TEST_AND (this->flags,SUBWINDOW_RIGHT)) {
      }
      ****/
  }
...
void    Main::keyboard (unsigned char key, int x, int y)
{
 ...
 (!active_control || !active_control->wants_tabs ())) {
    ...
    activate_control (new_control, ACTIVATE_TAB);
  }
...
void    Main::special (int key, int x, int y)
{
  curr_modifiers = glutGetModifiers ();
  ...
  if (button == GLUT_LEFT) {
      	 ...
      	 state == GLUT_UP) 
    {
      ...
      if (active_control AND 
           active_control->active_type == CONTROL_ACTIVE_MOUSEDOWN AND 0)
      {
      ...
      if (control) {
        if (NOT mouse_button_down AND state == GLUT_DOWN) {
          ...
          if (control->enabled) {
            activate_control (control, ACTIVATE_MOUSE);
...
void    Main::passive_motion (int x, int y)
{
  Control *control;
...
void    Main::entry (int state)
{
  /*if (NOT active_control OR (active_control AND (active_control->type == CONTROL_EDITTEXT
    OR active_control->type == CONTROL_SPINNER)))*/
  glutSetCursor (GLUT_CURSOR_LEFT_ARROW);
}
...
void    Main::visibility (int state)
{
}
...
Control  *Main::find_control (int x, int y)
{
  Control *node, *last_container;

  last_container = NULL;

  node = mainPanel;
  while (node != NULL) {
    if (!dynamic_cast<Column*> (node) AND
         PT_IN_BOX (x, y, 
...
) 
    {
      ...
      if (node->first_child () == NULL) {
        ...
        if (dynamic_cast<EditText*> (node)) {
          if (x < node->x_abs + ((EditText*)node)->text_x_offset)
...
void      Main::pack_controls (void)
{
  mainPanel->pack (0,0);

  /**** Now align controls within their bounds ****/
  align_controls (mainPanel);
  ...
  if (TEST_AND (this->flags, SUBWINDOW)) {
    ...
    if (1) {
      if (TEST_AND (this->flags,SUBWINDOW_TOP)) {
	mainPanel->w = MAX (mainPanel->w, parent_w);
      }
      else if (TEST_AND (this->flags,SUBWINDOW_LEFT)) {
	mainPanel->h = MAX (mainPanel->h, parent_h);
      }
      else if (TEST_AND (this->flags,SUBWINDOW_BOTTOM)) {
	mainPanel->w = MAX (mainPanel->w, parent_w);
      }
      else if (TEST_AND (this->flags,SUBWINDOW_RIGHT)) {
	mainPanel->h = MAX (mainPanel->h, parent_h);
...
void    Main::align_controls (Control *control)
{
  Control *child;

  control->align ();

  child = (Control*) control->first_child ();

  while (child != NULL) {
    align_controls (child);
    
    child = (Control*)child->next ();
  }  
}
...
/********************************* Main::post_update_main_gfx () ********/

void   Main::post_update_main_gfx (void)
{
  int old_window;
...
/** Return true if this control should redraw itself immediately (front buffer);
   ...
   Called from Control::redraw.
*/
bool	     Main::should_redraw_now (Control *ctl)
{
  switch (buffer_mode) {
...
/********************************* Main::set_current_draw_buffer () ********/

int          Main::set_current_draw_buffer (void)
{
  /* Save old buffer */
  GLint state;
...
/********************************* Main::restore_draw_buffer () **********/

void         Main::restore_draw_buffer (int buffer_state)
{
...
Main::Main (void) 
{
  ...
  mainPanel              = new Panel;
  mainPanel->set_int_val (PANEL_NONE);
  mainPanel->glui        = (GLUI*) this;
  mainPanel->name        = "\0";
}

/************************************ Main::draw_raised_box () **********/

void      Main::draw_raised_box (int x, int y, int w, int h)
{
  w = w+x;
  h = h+y;
...
/************************************ Main::draw_lowered_box () **********/
/* Not quite perfect...      **/

void      Main::draw_lowered_box (int x, int y, int w, int h)
{
  w = w+x;
  h = h+y;
...
/************************************* Main::activate_control () *********/

void         Main::activate_control (Control *control, int how)
{
  /** Are we not activating a control in the same window as the
    previous active control? */
  if (Master.active_control_glui AND
       this != (Main*) Master.active_control_glui) {
    Master.active_control_glui->deactivate_current_control ();
  }
  ...
  if (control != NULL AND control->can_activate AND control->enabled) {
    ...
    /*		active_control->type == CONTROL_ROLLOUT) {          */
  ...
  /*  printf ("activate: %d\n", glutGetWindow ());          */
  Master.active_control      = active_control;
  Master.active_control_glui = (GLUI*) this;
}


/************************* Main::deactivate_current_control () **********/

void         Main::deactivate_current_control (void)
{
  int orig;
  ...
  /*  printf ("deactivate: %d\n", glutGetWindow ());          */
  Master.active_control      = NULL;
  Master.active_control_glui = NULL;
}


/****************************** Main::find_next_control () **************/

Control  *Main::find_next_control_ (Control *control)
{
  /*** THIS IS NOT find_next_control ()!  This is an unused older
    version (look at the underscore at the end) ***/

  if (control == NULL)
    return find_next_control_rec (mainPanel);
  else
...
Control  *Main::find_next_control_rec (Control *control)
{
  Control *child = NULL, *rec_control, *sibling;
...
Control  *Main::find_next_control (Control *control)
{
  Control *tmp_control = NULL;
  int           back_up;
  ...
  while (control != NULL) {
    ...
    tmp_control = (Control*) control->first_child ();
    ...
    tmp_control = (Control*) control->next ();
...
/****************************** Main::find_prev_control () **************/

Control  *Main::find_prev_control (Control *control)
{
  Control *tmp_control, *next_control;

  if (control == NULL) {        /* here we find the last valid control */
    next_control = mainPanel;
    ...
    if (next_control == NULL OR tmp_control == mainPanel)
      return NULL;
    else 
      return tmp_control;
  }
...
/************************* Master_Object::set_glutIdleFunc () ***********/

void    Master_Object::set_glutIdleFunc (void (*f) (void))
{
  glut_idle_CB = f;
...
{
  mainPanel->sync_live (true, true);
}


/********************************* Master_Object::sync_live_all () *****/

void   Master_Object::sync_live_all (void) 
{
  GLUI *glui;
...
void   Master_Object::close_all (void) 
{
  GLUI *glui;
...
void   Main::close_internal (void) 
{
  glutDestroyWindow (glutGetWindow ()); /** Close this window **/

  this->unlink ();
  
  if (Master.active_control_glui == this) {
    Master.active_control      = NULL;
    Master.active_control_glui = NULL;
  }
...
/************************** Main::check_subwindow_position () **********/

void   Main::check_subwindow_position (void)
{
  ...
  if (TEST_AND (this->flags, SUBWINDOW)) {
    ...
    if (TEST_AND (this->flags,SUBWINDOW_RIGHT)) {
    ...
    GLUI *curr_glui;
    curr_glui = (GLUI*) Master.gluis.first_child (); 
    while (curr_glui) {
      if (TEST_AND (curr_glui->flags, SUBWINDOW) AND 
	   curr_glui->parent_window == this->parent_window) {

	if (TEST_AND (curr_glui->flags,SUBWINDOW_LEFT)) {
	}
	else if (TEST_AND (curr_glui->flags,SUBWINDOW_BOTTOM)) {
	}
	else if (TEST_AND (curr_glui->flags,SUBWINDOW_RIGHT)) {
	}
	else if (TEST_AND (curr_glui->flags,SUBWINDOW_TOP) AND 
		 (TEST_AND (this->flags,SUBWINDOW_LEFT) OR
		    TEST_AND (this->flags,SUBWINDOW_RIGHT))) {
	...
	/** We check the id's:  only the glui with the higher
	  ID number (meaning it was created later) gets bumped over **/
	if (curr_glui != this AND this->id > curr_glui->id) {
	  if (TEST_AND (this->flags,SUBWINDOW_LEFT) AND
	       TEST_AND (curr_glui->flags,SUBWINDOW_LEFT)) {
	    new_x += curr_glui->w;
	  }
	  else if (TEST_AND (this->flags,SUBWINDOW_TOP) AND
		    TEST_AND (curr_glui->flags,SUBWINDOW_TOP)) {
	    new_y += curr_glui->h;
	  }
	  else if (TEST_AND (this->flags,SUBWINDOW_BOTTOM) AND
		    TEST_AND (curr_glui->flags,SUBWINDOW_BOTTOM)) {
	    new_y -= curr_glui->h;
	  }
	  else if (TEST_AND (this->flags,SUBWINDOW_RIGHT) AND
		    TEST_AND (curr_glui->flags,SUBWINDOW_RIGHT)) {
	    new_x -= curr_glui->w;
	  }
...
/**************************** Master_Object::set_glutReshapeFunc () *****/

void Master_Object::set_glutReshapeFunc (void (*f) (int width, int height))
{
  glutReshapeFunc (reshape_func);
  add_cb_To_glut_window (glutGetWindow (), GLUT_RESHAPE, (void*) f);
}


/**************************** Master_Object::set_glutKeyboardFunc () ****/

void Master_Object::set_glutKeyboardFunc (void (*f) (unsigned char key, 
							int x, int y))
{
  glutKeyboardFunc (keyboard_func);
  add_cb_To_glut_window (glutGetWindow (), GLUT_KEYBOARD, (void*) f);
}


/*********************** Master_Object::set_glutSpecialFunc () **********/

void Master_Object::set_glutSpecialFunc (void (*f) (int key, 
						       int x, int y))
{
  glutSpecialFunc (special_func);
  add_cb_To_glut_window (glutGetWindow (), GLUT_SPECIAL, (void*) f);
}


/*********************** Master_Object::set_glutMouseFunc () **********/

void Master_Object::set_glutMouseFunc (void (*f) (int button, int state,
						     int x, int y))
{
  glutMouseFunc (mouse_func);
  add_cb_To_glut_window (glutGetWindow (), GLUT_MOUSE, (void*) f);
}


/****************************** parent_window_reshape_func () **********/
/* This is the reshape callback for a window that contains subwindows      */

void parent_window_reshape_func (int w, int h)
{
  int   current_window;
  GLUI  *glui;
  int   first = true;

  /*  printf ("parent_window_reshape_func: %d\n", glutGetWindow ());          */
...
/****************************** parent_window_keyboard_func () **********/

void parent_window_keyboard_func (unsigned char key, int x, int y)
{
  /*  printf ("parent_window_keyboard_func: %d\n", glutGetWindow ());          */
...
/****************************** parent_window_special_func () **********/

void parent_window_special_func (int key, int x, int y)
{
  /*printf ("parent_window_special_func: %d\n", glutGetWindow ());          */
...
/****************************** parent_window_mouse_func () **********/

void parent_window_mouse_func (int button, int state, int x, int y)
{
  int   current_window;
  GLUI  *glui;
...
/************************** Master_Object::find_glut_window () **********/

Glut_Window  *Master_Object::find_glut_window (int window_id)
{
  Glut_Window *window;

  window = (Glut_Window*) glut_windows.first_child ();
  while (window) {
    if (window->glut_window_id == window_id) 
      return window;
    
    window = (Glut_Window*) window->next ();
  }
...
/******************** Master_Object::Add_cb_To_glut_window () **********/

void     Master_Object::Add_cb_To_glut_window (int window_id,
						   int cb_type,void *cb)
{
  Glut_Window *window;
  ...
  if (NOT window) {
    ...
    window                 = new Glut_Window;
    window->glut_window_id = window_id;
    window->link_this_To_parent_last ((Node*) &this->glut_windows);
  }
...
/************* Master_Object::set_left_button_glut_menu_control () *****/

void  Master_Object::set_left_button_glut_menu_control (
							    Control *control)
{
...
/******************************* Main::set_ortho_projection () **********/

void  Main::set_ortho_projection (void)
{
  int win_h, win_w;
...
void  Main::set_viewport (void)
{
  glViewport (0, 0, mainPanel->w, mainPanel->h);
}


/****************************** Main::refresh () ****************/

void    Main::refresh (void)
{
  int orig;
...
/***************** Master_Object::get_main_gfx_viewport () ***********/

void     Master_Object::get_viewport_area (int *x, int *y, 
						int *w, int *h)
{
  GLUI *curr_glui;
  ...
  while (curr_glui) {
	...
	curr_w -= curr_glui->w;
      ...
      else if (TEST_AND (curr_glui->flags,SUBWINDOW_TOP)) {
	curr_h -= curr_glui->h;
      }
    }
...
/*****************Master_Object::auto_set_main_gfx_viewport () **********/

void           Master_Object::auto_set_viewport (void)
{
  int x, y, w, h;
...
void            GLUI::show (void)
{
  ...
  orig_window = mainPanel->set_To_glut_window ();

  glutShowWindow ();

  mainPanel->restore_window (orig_window);
}
