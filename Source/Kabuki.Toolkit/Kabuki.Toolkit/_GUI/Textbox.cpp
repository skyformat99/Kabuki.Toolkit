/****************************************************************************
  ...
  ---------------------------
     ...
     textbox.cpp - TextBox control class
...
TextBox::TextBox (Node *parent, String &live_var,
...
TextBox::TextBox (Node *parent, bool scroll, int id,
                            CB callback)
{
  common_construct (parent, NULL, scroll, id, callback);
}

/****************************** TextBox::common_construct () **********/
void TextBox::common_construct (
  ...
  bool scroll, int id, CB callback)
{
  ...
  if (data) {
    ...
    this->live_type = LIVE_NONE;
  }
  this->user_id     = id;
  this->callback    = callback;
  this->name        = "textbox";
  tbPanel->add_control (this);
  if (scroll) {
    new Column (tbPanel, false);
    scrollbar = 
      new Scrollbar (tbPanel,
                         "scrollbar",
                         SCROLL_VERTICAL,
                         SCROLL_INT);
    scrollbar->set_object_callback (TextBox::scrollbar_callback, this);
...
/****************************** TextBox::mouse_down_handler () **********/

int    TextBox::mouse_down_handler (int local_x, int local_y)
{
...
/******************************** TextBox::mouse_up_handler () **********/

int    TextBox::mouse_up_handler (int local_x, int local_y, bool inside)
{
  return false;
}


/***************************** TextBox::mouse_held_down_handler () ******/

int    TextBox::mouse_held_down_handler (int local_x, int local_y,
                           bool new_inside)
{
  int tmp_pt;
...
/****************************** TextBox::key_handler () **********/
int    TextBox::key_handler (unsigned char key,int modifiers)
{
  int regular_key;
  /* int has_selection;              */
 ...
 ((key == 'd') AND (modifiers == GLUT_ACTIVE_ALT)))
  ...
  else if (modifiers == GLUT_ACTIVE_CTRL)  /* CTRL ONLY */ 
  {
    ...
    if (key == CTRL ('a')) {
      ...
      sel_start = sel_end = insertion_pt;
      text.erase (insertion_pt,String::npos);
    }
  }
...
/************************** TextBox::update_substring_bounds () *********/

int    TextBox::update_substring_bounds (void)
{
  int box_width;
...
/********************************* TextBox::update_x_offsets () *********/

void    TextBox::update_x_offsets (void)
{
}
...
void    TextBox::draw_text (int x, int y)
{
  DRAWINGSENTINAL_IDIOM
  ...
  text_x = 2 + TEXTBOX_BOXINNERMARGINX;
...
int  TextBox::find_insertion_pt (int x, int y)
{
  ...
  int y_off = y - (y_abs + 2 + TEXTBOX_BOXINNERMARGINX);
  int x_off = x - (x_abs + 2 + TEXTBOX_BOXINNERMARGINX);
  ...
  int curr_x = this->x_abs 
    ...
    + TEXTBOX_BOXINNERMARGINX;   /** plus this many pixels blank space
...
int TextBox::get_box_width () 
{
  return MAX (this->w 
              - 4     /*  2 * the two-line box border */ 
              - 2 * TEXTBOX_BOXINNERMARGINX, 0);

}

/******************************** TextBox::draw_insertion_pt () *********/

void     TextBox::draw_insertion_pt (void)
{
  ...
  if (insertion_pt == text.length () && text[text.length ()-1] == '\n'
      || curr_x-this->x_abs > (w - 2 - TEXTBOX_BOXINNERMARGINX)) { // Insert on the next line 
    curr_x = this->x_abs + TEXTBOX_BOXINNERMARGINX;
    line++;
  } 
...
/******************************** TextBox::substring_width () *********/
int  TextBox::substring_width (int start, int end, int initial_width)
{
...
/***************************** TextBox::update_and_draw_text () ********/

void   TextBox::update_and_draw_text (void)
{
  //update_substring_bounds ();
...
/********************************* TextBox::special_handler () **********/

int    TextBox::special_handler (int key,int modifiers)
{
  int tmp_insertion_pt;
  if (NOT glui)
    return false;
...
int    TextBox::find_word_break (int start, int direction)
{
  int    i, j;
  char    breaks[] = " \n\t:-.,";
...
/********************************** TextBox::clear_substring () ********/

void   TextBox::clear_substring (int start, int end)
{
...
void   TextBox::update_size (void)
{
  ...
  if (w < TEXTBOX_MIN_TEXT_WIDTH)
      w = TEXTBOX_MIN_TEXT_WIDTH;
}
...
void   TextBox::dump (FILE *out, char *name)
{
  fprintf (out, 
...
int    TextBox::mouse_over (int state, int x, int y)
{
  if (state && enabled) {
...
void TextBox::scrollbar_callback (Control *my_scrollbar) {
  Scrollbar *sb = dynamic_cast<Scrollbar*> (my_scrollbar);
  if (!sb) return;
  TextBox* me = (TextBox*) sb->associated_object;
