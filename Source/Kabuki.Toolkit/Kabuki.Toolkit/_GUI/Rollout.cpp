...
enum {rollout_height_pixels=DEFAULT_CONTROL_HEIGHT + 7};

/****************************** Rollout::Rollout () **********/

Rollout::Rollout (Node *parent, const char *name, 
                            int open, int type)
{
  common_init ();
  set_name (name);
...
void    Rollout::open (void)
{
  ...
  /* Copy hidden children into our list "collapsed_node" */
  ...
  if (child_head != NULL) {
 ((Control*) child_head)->unhide_internal (true);
  }
...
void    Rollout::close (void)
{
  ...
  if (child_head != NULL) {
 ((Control*) child_head)->hide_internal (true);
  }

  /* Move all children into a list of hidden children */
...
/**************************** Rollout::mouse_down_handler () **********/


int   Rollout::mouse_down_handler (int local_x, int local_y)
{
...
/**************************** Rollout::mouse_held_down_handler () ****/

int  Rollout::mouse_held_down_handler (
...
int   Rollout::mouse_up_handler (int local_x, int local_y, bool inside)
{
  if (currently_inside) {    
    if (is_open)
      close ();
...
/**************************** Rollout::draw_pressed () ***********/

void   Rollout::draw_pressed (void)
{
  int left, right, top, bottom;
