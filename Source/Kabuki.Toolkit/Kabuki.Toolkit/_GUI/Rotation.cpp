/****************************************************************************
  ...
  ---------------------------
     ...
     rotation - Rotation control class
...
/*************************** Rotation::iaction_mouse_down_handler () ***/

int    Rotation::iaction_mouse_down_handler (int local_x, int local_y)
{
...
/*********************** Rotation::iaction_mouse_up_handler () **********/

int    Rotation::iaction_mouse_up_handler (int local_x, int local_y, 
						bool inside)
{
  copy_float_array_To_ball ();
...
/******************* Rotation::iaction_mouse_held_down_handler () ******/

int    Rotation::iaction_mouse_held_down_handler (int local_x, int local_y,
						       bool inside)
{  
  if (NOT glui)
    return 0;
...
/******************** Rotation::iaction_draw_active_area_persp () **************/

void    Rotation::iaction_draw_active_area_persp (void)
{
  /********** arcball *******/
  copy_float_array_To_ball ();
...
/******************** Rotation::iaction_draw_active_area_ortho () **********/

void    Rotation::iaction_draw_active_area_ortho (void)
{
  float radius;
...
/******************************** Rotation::iaction_dump () **********/

void     Rotation::iaction_dump (FILE *output)
{
}


/******************** Rotation::iaction_special_handler () **********/

int    Rotation::iaction_special_handler (int key,int modifiers)
{
...
/****************************** Rotation::setup_texture () *********/

void Rotation::setup_texture (void)
{
...
/****************************** Rotation::setup_lights () ***********/

void    Rotation::setup_lights (void)
{
...
void    Rotation::draw_ball (float radius)
{
  if (NOT can_draw ())
    return;
...
bool Rotation::needs_idle (void) const
{
  return can_spin;
}
...
/********************** Rotation::copy_float_array_To_ball () *********/

void     Rotation::copy_float_array_To_ball (void)
{
  int i;
  float *fp_src, *fp_dst;
...
/********************** Rotation::copy_ball_To_float_array () *********/

void     Rotation::copy_ball_To_float_array (void)
{
  mat4 tmp_rot;
  tmp_rot = *ball->rot_ptr;
...
void   Rotation::set_spin (float damp_factor)
{
  if (damp_factor == 0.0) 
    can_spin = false;
  else
...
/************** Rotation::common_init () ********************/

void Rotation::common_init (void) 
{
  format_str (name, "Rotation: %p", this);
//  type                = CONTROL_ROTATION;
  w                   = ROTATION_WIDTH;
  h                   = ROTATION_HEIGHT;
  can_activate        = true;
  live_type           = LIVE_FLOAT_ARRAY;
  float_array_size    = 16;
  quadObj             = NULL;
