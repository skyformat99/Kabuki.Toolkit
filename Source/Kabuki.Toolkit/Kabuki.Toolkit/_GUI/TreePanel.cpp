/*
  This software is provided 'as-is', without any express or implied 
  warranty. In no event will the authors be held liable for any damages 
  arising from the use of this software. 

  Permission is granted to anyone to use this software for any purpose, 
  including commercial applications, and to alter it and redistribute it 
  freely, subject to the following restrictions: 

  1. The origin of this software must not be misrepresented; you must not 
  claim that you wrote the original software. If you use this software 
  in a product, an acknowledgment in the product documentation would be 
  appreciated but is not required. 
  2. Altered source versions must be plainly marked as such, and must not be 
  misrepresented as being the original software. 
  3. This notice may not be removed or altered from any source distribution. 
*/

#include "../Public/_GUI/UI.h"



/****************************** TreePanel::TreePanel () *********/

TreePanel::TreePanel (Node *parent, const char *name, 
                               bool open, int inset)
{
  common_init ();

  set_name (name);
  user_id    = -1;
        
  if (!open) {
    is_open = false;
    h = DEFAULT_CONTROL_HEIGHT + 7;
  }

  parent->add_control (this);
}

/****************************** TreePanel::set_color () *********/

void TreePanel::set_color (float r, float g, float b) 
{
  red = r;
  green = g;
  blue = b;
  redraw ();
}

/************************ TreePanel::set_level_color () *********/

void TreePanel::set_level_color (float r, float g, float b) 
{
  lred = r;
  lgreen = g;
  lblue = b;
  redraw ();
}

/****************************** TreePanel::ab () *********/

/* Adds branch to curr_root */
Tree *TreePanel::ab (const char *name, Tree *root) 
{
  Tree *temp;

  
  if (root != NULL) {
    resetToRoot (root);
  }

  temp = new Tree (curr_root, name);
  initNode (temp);
  formatNode (temp);

  curr_root = temp;
  curr_branch = NULL; /* Currently at leaf */

  if (dynamic_cast<Tree*> (temp))
 ((Tree *)temp)->set_current (true);
  //refresh ();
  //  glui->deactivate_current_control ();
  //glui->activate_control (temp, ACTIVATE_TAB);
  return temp;

}

/****************************** TreePanel::fb () *********/

/* Goes up one level, resets curr_root and curr_branch to parents*/
void TreePanel::fb (Tree *branch) 
{
  if (((Panel *)branch) == ((Panel *)this))
    return;

  if (((Panel *)curr_branch) == ((Panel *)this)) {
    resetToRoot ();
    return;
  }
  if (((Panel *)curr_root) == ((Panel *)this)) {
    resetToRoot ();
    return;
  }

  if (branch != NULL) {

    if (dynamic_cast<Tree*> (branch))
 ((Tree *)branch)->set_current (false);

    curr_branch = (Tree *)branch->next ();
    curr_root = (Panel *)branch->parent ();

    if (curr_branch == NULL && (curr_root->collapsed_node).first_child () != NULL)
      curr_branch = (Tree *) (curr_root->collapsed_node).first_child ();

    if (dynamic_cast<Tree*> (curr_root))
 ((Tree *)curr_root)->set_current (true);

  } else {
    if (curr_root != NULL) { /* up one parent */

      if (dynamic_cast<Tree*> (curr_root))
	 ((Tree *)curr_root)->set_current (false);

      curr_branch = (Tree *) curr_root->next ();
      curr_root = (Panel *) curr_root->parent ();

      if (curr_branch == NULL && (curr_root->collapsed_node).first_child () != NULL)
	curr_branch = (Tree *) (curr_root->collapsed_node).first_child ();

      if (dynamic_cast<Tree*> (curr_root))
	 ((Tree *)curr_root)->set_current (true);

    }

  }
  //refresh ();
}


/****************************** TreePanel::refresh () *********/

void TreePanel::refresh () 
{
  glui->deactivate_current_control ();
  glui->activate_control (curr_root, ACTIVATE_TAB);

  redraw ();
}

/****************************** TreePanel::initNode () *********/

void TreePanel::initNode (Tree *temp) 
{
  if (temp == NULL)
    return;
  int level = temp->get_level ();
  int child_number = 1;

  Tree *ptree = dynamic_cast<Tree*> (temp->parent ());
  if (ptree) {
    level = ptree->get_level () + 1;
    Tree *prevTree = dynamic_cast<Tree*> (temp->prev ());
    if (prevTree) {
      child_number = prevTree->get_child_number () + 1;
    }
  } else if (dynamic_cast<Tree*> (temp) && 
             dynamic_cast<TreePanel*> (temp->parent ())) {
    child_number = ++root_children;
  }
  temp->set_id (uniqueID ());     // -1 if unset
  temp->set_level (level);
  temp->set_child_number (child_number);
}

/****************************** TreePanel::formatNode () *********/

void TreePanel::formatNode (Tree *temp) 
{
  if (temp == NULL)
    return;
  int level = temp->get_level ();
  int child_number = temp->get_child_number ();
  String level_name="";
  String full_name="";

  temp->level_name == "";

  if (format & TREEPANEL_DISPLAY_HIERARCHY) {
    if (format & TREEPANEL_HIERARCHY_LEVEL_ONLY) {
      format_str (level_name, "%d", level);
    }
    if (format & TREEPANEL_HIERARCHY_NUMERICDOT) {
      if (dynamic_cast<Tree*> (temp->parent ()))
        format_str (level_name, "%s.%d", 
 ((Tree *) (temp->parent ()))->level_name.c_str (), 
                        child_number);
      else
        format_str (level_name, "%d", child_number);
    }
  }

  temp->set_level_color (lred, lgreen, lblue);
  temp->set_format (format);
  temp->level_name = level_name;

  if (format & TREEPANEL_ALTERNATE_COLOR) {
    switch (level%8) {
    case (7): temp->set_color (.5,.5,.5); break;
    case (6): temp->set_color (.3,.5,.5); break;
    case (5): temp->set_color (.5,.3,.5); break;
    case (4): temp->set_color (.3,.3,.5); break;
    case (3): temp->set_color (.5,.5,.3); break;
    case (2): temp->set_color (.3,.5,.3); break;
    case (1): temp->set_color (.5,.3,.3); break;
    default: temp->set_color (.3,.3,.3);
    }
  } else {
    temp->set_color (red,green,blue);
  }

  if (format & TREEPANEL_DISABLE_BAR) {
    temp->disable_bar ();
  } else {
    if (format & TREEPANEL_DISABLE_DEEPEST_BAR) {
      temp->disable_bar ();
      if (dynamic_cast<Tree*> (curr_root))
 ((Tree *)curr_root)->enable_bar ();
    } else
      if (format & TREEPANEL_CONNECT_CHILDREN_ONLY) {
        temp->disable_bar ();
        if (temp->prev () && dynamic_cast<Tree*> (temp->prev ())) 
        {
 ((Tree *)temp->prev ())->enable_bar ();
        }
      }
  }
}

/****************************** TreePanel::update_all () *********/

void TreePanel::update_all () 
{
  printf ("TreePanel::update_all () doesn't work yet. - JVK\n");
  return;
  Panel *saved_root = curr_root;
  Tree *saved_branch = curr_branch;
  root_children = 0;
  resetToRoot (this);
  if (curr_branch && dynamic_cast<Tree*> (curr_branch))
    formatNode ((Tree *)curr_branch);
  next ();
  while (curr_root && curr_branch != this->first_child ()) {
    if (curr_branch && dynamic_cast<Tree*> (curr_branch)) {
      formatNode ((Tree *)curr_branch);
    }
    next ();
  } 
  curr_root = saved_root;
  curr_branch = saved_branch;
}

/****************************** TreePanel::expand_all () *********/

void            TreePanel::expand_all () 
{
  Panel *saved_root = curr_root;
  Tree *saved_branch = curr_branch;

  resetToRoot (this);
  if (dynamic_cast<Tree*> (curr_root))
 ((Tree*)curr_root)->open ();
  next ();
  while (curr_root != NULL && curr_branch != this->first_child ()) {
    if (dynamic_cast<Tree*> (curr_root))
 ((Tree*)curr_root)->open ();
    next ();
  }

  curr_root = saved_root;
  curr_branch = saved_branch;
}

/****************************** TreePanel::collapse_all () *********/

void            TreePanel::collapse_all () 
{
  Panel *saved_root = curr_root;
  Tree *saved_branch = curr_branch;

  resetToRoot (this);
  next ();
  while (curr_root != NULL && curr_branch != this->first_child ()) {
    if (dynamic_cast<Tree*> (curr_root) && 
	      curr_branch == NULL) { /* we want to close everything leaf-first */
 ((Tree*)curr_root)->close ();
      /* Rather than simply next (), we need to manually move the
         curr_root because this node has been moved to the 
         collapsed_node list */
      curr_branch = (Tree *)curr_root->next ();
      curr_root = (Panel *)curr_root->parent ();
    } else
      next ();
  }

  curr_root = saved_root;
  curr_branch = saved_branch;

}

/****************************** TreePanel::db () *********/

/* Deletes the curr_root */
void TreePanel::db (Tree *root) 
{
  Tree  *temp_branch;
  Panel *temp_root;

  if (((Control *)root) == ((Control *)this))
    return;

  if (root != NULL) {
    curr_root = (Tree *)root;
    curr_branch = NULL;
  }

  if (curr_root == NULL || ((Panel *)curr_root) == ((Panel *)this)) {
    resetToRoot ();
    return;
  }


  temp_branch = (Tree *)curr_root->next ();              /* Next branch, if any */
  temp_root   = (Panel *)curr_root->parent ();      /* new root */
  curr_root->unlink ();
  delete curr_root;
  curr_branch = (Tree *) temp_branch;
  curr_root   = (Panel *) temp_root;
  if (dynamic_cast<Tree*> (curr_root))
 ((Tree *)curr_root)->open ();

  if ((format & TREEPANEL_DISABLE_DEEPEST_BAR) == TREEPANEL_DISABLE_DEEPEST_BAR) {
    if (dynamic_cast<Tree*> (curr_root) && ((Tree *)curr_root->next ()) == NULL)
 ((Tree *)curr_root)->disable_bar ();
  }
  //refresh ();
}

/****************************** TreePanel::descendBranch () *********/

/* Finds the very last branch of curr_root, resets vars */
void            TreePanel::descendBranch (Panel *root) {
  if (root)
    resetToRoot (root);
  else
    resetToRoot (curr_root);
  if (curr_branch != NULL && curr_branch != ((Panel *)this)) {
    if (dynamic_cast<Tree*> (curr_root))
 ((Tree *)curr_root)->set_current (false);
    descendBranch (curr_branch); 
  } 
}

/****************************** TreePanel::next () *********/

void TreePanel::next () 
{
  if (curr_root == NULL)
    resetToRoot (this);

  if (curr_branch == NULL && (curr_root->collapsed_node).first_child () != NULL)
    curr_branch = (Tree *) (curr_root->collapsed_node).first_child ();


  if (curr_branch != NULL && curr_branch != ((Panel *)this)) {     /* Descend into branch */
    if (dynamic_cast<Tree*> (curr_root))
 ((Tree *)curr_root)->set_current (false);
    resetToRoot (curr_branch);
  } else if (curr_branch == NULL) {
    fb (NULL);  /* Backup and move on */
  }
}

/****************************** TreePanel::resetToRoot () *********/

/* Resets curr_root and curr branch to TreePanel and lastChild */
void TreePanel::resetToRoot (Panel *new_root) 
{
  Panel *root = this;
  if (new_root != NULL)
    root = new_root;
  curr_root = root;
  if (dynamic_cast<Tree*> (curr_root))
 ((Tree *)curr_root)->set_current (true);
  curr_branch = (Tree *)root->first_child ();

  /* since Trees are collapsable, we need to check the collapsed nodes
     in case the curr_root is collapsed */
  if (curr_branch == NULL && (root->collapsed_node).first_child () != NULL) {
    curr_branch = (Tree *) (root->collapsed_node).first_child ();
  }
  while (curr_branch && dynamic_cast<Tree*> (curr_branch)) {
    curr_branch= (Tree *)curr_branch->next ();
  }
}
