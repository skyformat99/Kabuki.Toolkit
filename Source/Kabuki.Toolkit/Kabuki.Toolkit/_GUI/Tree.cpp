...
/****************************** Tree::Tree () **********/
Tree::Tree (Node *parent, const char *name, 
                     int open, int inset)
{
  common_init ();
  StaticText *inset_label;
  Column     *col;
  ...
  if (NOT open) {
    this->is_open = false;
    this->h = DEFAULT_CONTROL_HEIGHT + 7;
  }

  parent->add_control (this);
  inset_label = new StaticText (this,"");
  inset_label->set_w (inset);
  col = new Column (this,true);
  this->set_column (col);
  this->set_alignment (ALIGN_LEFT);
}
...
/**************************** Tree::mouse_down_handler () **********/


int   Tree::mouse_down_handler (int local_x, int local_y)
{
  if (local_y - y_abs > 18) {
...
/**************************** Tree::mouse_held_down_handler () ****/

int  Tree::mouse_held_down_handler (
...
int   Tree::mouse_up_handler (int local_x, int local_y, bool inside)
{
  if (currently_inside) {    
    if (is_open)
      close ();
