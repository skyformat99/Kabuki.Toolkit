...
void    Panel::Draw (int x, int y)
{
    ...
    **/    
    ...
    else if (int_val == PANEL_EMBOSSED)
    {
    if (parent_node == NULL || name == "") {
      top = 0;
    }
    else {
      top = PANEL_EMBOSS_TOP;
    }
    ...
    if (parent_node != NULL && name != "") { /* Only  Draw non-null strings */
      int left = 7, height=PANEL_NAME_DROP+1;
      int str_width;
	...
	glColor3ubv (glui->bkgd_color);
      ...
      draw_name (left, PANEL_NAME_DROP);
    }
  }
...
void    Panel::set_name (const char *new_name)
{
...
void   Panel::update_size (void)
{
  ...
  if (name != "" AND int_val == PANEL_EMBOSSED) {
    this->y_off_Top = YOFF + 8;
  }
  else {
    this->y_off_Top = YOFF;
  }
