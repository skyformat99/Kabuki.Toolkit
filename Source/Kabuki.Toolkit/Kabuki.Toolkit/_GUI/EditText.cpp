/****************************************************************************
  ...
  ---------------------------
     ...
     edittext.cpp - EditText control class
...
EditText::EditText (Node *parent, const char *name,
                             ...
                             int id, CB callback)
{
  if (data_type == EDITTEXT_TEXT) {
    live_type = LIVE_TEXT;
  }
  else if (data_type == EDITTEXT_STRING) {
   data_type = EDITTEXT_TEXT;  // EDITTEXT_STRING doesn't really exist.
                                     // Except as a signal to make a string.
                                     // It's a backwards-compat hack.
   live_type = LIVE_STRING;
  }
  else if (data_type == EDITTEXT_INT) {
    live_type = LIVE_INT;
  }
  else if (data_type == EDITTEXT_FLOAT) {
    live_type = LIVE_FLOAT;
  }
...
EditText::EditText (Node *parent, const char *name,
                              int text_type, int id, CB callback)
{
  common_construct (parent, name, text_type, LIVE_NONE, 0, id, callback);
}
...
EditText::EditText (Node *parent, const char *name,
                              ...
                              int id, CB callback)
{
  common_construct (parent, name, EDITTEXT_INT, LIVE_INT, live_var, id, callback);
}
...
EditText::EditText (Node *parent, const char *name,
                              ...
                              int id, CB callback)
{
  common_construct (parent, name, EDITTEXT_FLOAT, LIVE_FLOAT, live_var, id, callback);
}
...
EditText::EditText (Node *parent, const char *name, 
                              ...
                              int id, CB callback)
{
  common_construct (parent, name, EDITTEXT_TEXT, LIVE_TEXT, live_var, id, callback);
}
...
EditText::EditText (Node *parent, const char *name, 
                              ...
                              int id, CB callback)
{
  common_construct (parent, name, EDITTEXT_TEXT, LIVE_STRING, &live_var, id, callback);
}
                                      ...
                                      CB cb)
{
  ...
  if (live_type == LIVE_INT) {
    if (data == NULL)
      set_int_val (int_val);   /** Set to some default, in case of no live auto **/
  }
  else if (live_type == LIVE_FLOAT) {
    num_periods = 1;
    if (data == NULL)
      set_float_val (float_val);   /** Set to some default, in case of no live auto **/
  }
...
int    EditText::key_handler (unsigned char key,int modifiers)
{
  ...
  if (key == CTRL ('m')) {           /* RETURN */
    ...
    activate (ACTIVATE_TAB);     /** Reselect all text **/
    redraw ();
    return true;
  }
 ...
 (key == CTRL ('d') AND modifiers == GLUT_ACTIVE_CTRL)) 
  {
    if (sel_start == sel_end) {   /* no selection */
      if (insertion_pt < (int)text.length ()) {
        ...
        if (data_type == EDITTEXT_FLOAT AND text[insertion_pt]=='.')
          num_periods--;
 ...
 ((key == 'd') AND (modifiers == GLUT_ACTIVE_ALT)))
  {
    ...
    if (sel_start == sel_end) {   /* no selection */
      if (insertion_pt > 0) {
        /*** See if we're deleting a period in a float data-type box ***/
        if (data_type == EDITTEXT_FLOAT AND text[insertion_pt-1]=='.')
          num_periods--;
...
void    EditText::Draw (int x, int y)
{
...
void    EditText::draw_text (int x, int y)
{
  ...
  text_x = text_x_offset + 2 + EDITTEXT_BOXINNERMARGINX;
...
int  EditText::find_insertion_pt (int x, int y)
{
  ...
  curr_x = this->x_abs + text_x_offset 
    ...
    + EDITTEXT_BOXINNERMARGINX;   /** plus this many pixels blank space
...
void   EditText::update_size (void)
{
  ...
  if (data_type == EDITTEXT_TEXT OR 
       data_type == EDITTEXT_FLOAT) {
    if (w < text_x_offset + EDITTEXT_MIN_TEXT_WIDTH)
      w = text_x_offset + EDITTEXT_MIN_TEXT_WIDTH;
  }
  else if (data_type == EDITTEXT_INT) {
    if (w < text_x_offset + EDITTEXT_MIN_INT_WIDTH)
      w = text_x_offset + EDITTEXT_MIN_INT_WIDTH;
  }
}
...
void EditText::set_float_limits (float low, float high, int limit_type)
{
...
void   EditText::set_int_limits (int low, int high, int limit_type)
{
  has_limits  = limit_type;
  int_low     = low;
  int_high    = high;
